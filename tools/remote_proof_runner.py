...
try:
    ...
    from tools.suite_control import (COMPATIBILITY_PROPOSALS, candidate_tuple_path,
                                     configure_source_roots, load_compatibility_proposal,
                                     load_release_candidate_tuple)  # noqa: E402
    from tools.suite_protocol import protocol_digest  # noqa: E402
    ...
    COMPATIBILITY_PROPOSALS = None
    candidate_tuple_path = configure_source_roots = load_compatibility_proposal = load_release_candidate_tuple = protocol_digest = None
...
PRODUCTS = ("om", "mom", "nona", "omega")
MAX_FILES, MAX_FILE, MAX_TOTAL, CHUNK_SIZE = 200_000, 2 << 30, 8 << 30, 80 << 20
CHUNK_INDEX_SCHEMA = "suite.remote_chunk_index"
PROPOSAL_FIELDS = frozenset({"schema", "major", "cid", "protocol", "products", "compatibility", "target_platforms", "runtime_closures"})
CANDIDATE_FIELDS = frozenset({"schema", "major", "cid", "product", "version", "vcs", "source_digest", "artifacts", "origins_sha256", "protocol", "state_contract", "compatibility", "platforms", "previous_candidate"})
...
def verify_driver_interface(repo: Path, commit: str) -> None:
    """Execute the tuple-pinned clean-reproduction parser, never working-tree code."""
    with tempfile.TemporaryDirectory(prefix="fusion-remote-driver-") as raw:
        root = Path(raw)
        prefix = "sources/fusion/repo/"
        for name, mode, body in source_entries(repo, commit, "fusion"):
            if not name.startswith(prefix):
                raise RemoteProofError("remote.driver_incompatible")
            destination = root / name[len(prefix):]
            write_once(destination, body, mode)
        driver = root / "tools" / "clean_reproduce.py"
        if not driver.is_file() or driver.is_symlink():
            raise RemoteProofError("remote.driver_incompatible")
        completed = subprocess.run(
            (sys.executable, str(driver), "--help"), cwd=root, check=False,
            stdin=subprocess.DEVNULL, capture_output=True, timeout=60,
            env={"PATH": "/usr/bin:/bin", "LC_ALL": "C", "PYTHONDONTWRITEBYTECODE": "1"},
        )
        required = (b"--release-input-root", b"--tuple", b"--platform",
                    b"--runtime-closure-root", b"--output")
        if completed.returncode or any(option not in completed.stdout for option in required):
            raise RemoteProofError("remote.driver_incompatible")


def harden_runtime_closure_cas(root: Path) -> None:
    """Restore the private immutable CAS modes required by release verification."""
    runtime_root = root / "runtime-closures"
    if not runtime_root.exists():
        return
    if not runtime_root.is_dir() or runtime_root.is_symlink():
        raise RemoteProofError("remote.runtime_closure_invalid")
    required = [runtime_root]
    packages = runtime_root / "packages"
    if packages.exists():
        required.append(packages)
    for product in PRODUCTS:
        for directory in (runtime_root / product, packages / product):
            if directory.exists():
    ...
    for path in runtime_root.rglob("*"):
        if path.is_symlink():
            raise RemoteProofError("remote.runtime_closure_invalid")
        if path.is_file():
            if path.suffix not in {".json", ".tar"}:
                raise RemoteProofError("remote.runtime_closure_invalid")
            path.chmod(0o444)
...
def chunked_input(data: bytes, root: Path) -> tuple[Path, str]:
    """Write one immutable content-addressed payload as GitHub-safe tracked chunks."""
    if not data or len(data) > MAX_TOTAL:
        raise RemoteProofError("remote.transfer_too_large")
    ...
    for index, offset in enumerate(range(0, len(data), CHUNK_SIZE)):
        ...
        relative = f"input/transfer-{sha}.part-{index:04d}"
        write_once(root / relative, body)
        chunks.append({"path": relative, "size": len(body),
                       "sha256": hashlib.sha256(body).hexdigest()})
    value: dict[str, object] = {
        "schema": CHUNK_INDEX_SCHEMA, "major": 1, "content_sha256": sha,
        "content_size": len(data), "chunk_size": CHUNK_SIZE, "chunks": chunks,
    }
    value["cid"] = content_cid(value)
    path = root / "input" / f"transfer-{sha}.parts.json"
    write_once(path, canonical(value) + b"\n")
    return path, sha


def assemble_transfer(index: Path, expected_sha: str, output: Path) -> dict[str, object]:
    """Reassemble a canonical chunk index without trusting filenames or ambient files."""
    if not SHA.fullmatch(expected_sha):
        raise RemoteProofError("remote.artifact_digest_invalid")
    index = index.resolve(strict=True)
    root = index.parent.parent.resolve(strict=True)
    if index.is_symlink() or index.parent.name != "input" or index.name != f"transfer-{expected_sha}.parts.json":
        raise RemoteProofError("remote.chunk_index_invalid")
    raw = index.read_bytes()
    value = strict_json(raw, "remote.chunk_index_invalid")
    fields = {"schema", "major", "cid", "content_sha256", "content_size", "chunk_size", "chunks"}
    if (not isinstance(value, dict) or set(value) != fields
            or value.get("schema") != CHUNK_INDEX_SCHEMA or value.get("major") != 1
            or value.get("cid") != content_cid(value) or value.get("content_sha256") != expected_sha
            or value.get("chunk_size") != CHUNK_SIZE or canonical(value) + b"\n" != raw
            or not isinstance(value.get("content_size"), int) or not 0 < value["content_size"] <= MAX_TOTAL
            or not isinstance(value.get("chunks"), list) or not value["chunks"]):
        raise RemoteProofError("remote.chunk_index_invalid")
    assembled = bytearray()
    previous = ""
    for position, item in enumerate(value["chunks"]):
        relative = f"input/transfer-{expected_sha}.part-{position:04d}"
        if (not isinstance(item, dict) or set(item) != {"path", "size", "sha256"}
                or item.get("path") != relative or relative <= previous
                or not isinstance(item.get("size"), int) or not 0 < item["size"] <= CHUNK_SIZE
        ...
        try:
            ...
            raise RemoteProofError("remote.chunk_missing", relative) from exc
        if (not stat.S_ISREG(info.st_mode) or path.is_symlink() or info.st_nlink != 1
                or len(body) != item["size"] or hashlib.sha256(body).hexdigest() != item["sha256"]):
            raise RemoteProofError("remote.chunk_invalid", relative)
        assembled.extend(body)
        if len(assembled) > MAX_TOTAL:
            raise RemoteProofError("remote.transfer_too_large")
    data = bytes(assembled)
    if len(data) != value["content_size"] or hashlib.sha256(data).hexdigest() != expected_sha:
        raise RemoteProofError("remote.artifact_digest_mismatch")
    write_once(output.resolve(), data)
    return {"source_sha256": expected_sha, "size": len(data), "chunks": len(value["chunks"]),
...
def verify_transfer_bytes(data: bytes) -> dict[str, object]:
    ...
    verify_release_inputs(by_name, manifest)
    return manifest

def verify_release_inputs(by_name: Mapping[str, tuple[int, bytes]], manifest: Mapping[str, object]) -> dict[str, object]:
    """Bind every transferred release input to the manifest, tuple, and source identities."""
    tuple_cid = str(manifest["candidate_tuple"])
    tuple_name = f"release-inputs/candidate-tuples/{tuple_cid}.json"
    tuple_entry = by_name.get(tuple_name)
    if tuple_entry is None:
        raise RemoteProofError("remote.release_input_missing", tuple_name)
    candidate_tuple = strict_json(tuple_entry[1], "remote.tuple_invalid")
    tuple_fields = {"schema", "major", "cid", "protocol", "supervisor", "products", "compatibility_proposal"}
    if (not isinstance(candidate_tuple, dict) or set(candidate_tuple) != tuple_fields
            or candidate_tuple.get("schema") != "suite.release_candidate_tuple"
            or candidate_tuple.get("major") != 1 or candidate_tuple.get("cid") != tuple_cid
            or content_cid(candidate_tuple) != tuple_cid or not CID.fullmatch(str(candidate_tuple.get("protocol", "")))):
        raise RemoteProofError("remote.tuple_invalid")
    products = candidate_tuple.get("products")
    supervisor = candidate_tuple.get("supervisor")
    sources = manifest.get("sources")
    if (not isinstance(products, dict) or set(products) != set(PRODUCTS)
            or any(not CID.fullmatch(str(value)) for value in products.values())
            or not isinstance(supervisor, dict) or set(supervisor) != {"commit", "tree"}
            or not isinstance(sources, dict) or not isinstance(sources.get("fusion"), dict)
            or any(supervisor.get(key) != sources["fusion"].get(key) for key in ("commit", "tree"))):
        raise RemoteProofError("remote.tuple_source_mismatch")
    proposal_cid = str(candidate_tuple.get("compatibility_proposal", ""))
    proposal_name = f"release-inputs/compatibility-proposals/{proposal_cid}.json"
    proposal_entry = by_name.get(proposal_name)
    if not CID.fullmatch(proposal_cid) or proposal_entry is None:
        raise RemoteProofError("remote.release_input_missing", proposal_name)
    proposal = strict_json(proposal_entry[1], "remote.proposal_invalid")
    if (not isinstance(proposal, dict) or set(proposal) != PROPOSAL_FIELDS
            or proposal.get("schema") != "suite.compatibility_proposal" or proposal.get("major") != 1
            or proposal.get("cid") != proposal_cid or content_cid(proposal) != proposal_cid
            or proposal.get("protocol") != candidate_tuple["protocol"] or proposal.get("products") != products):
        raise RemoteProofError("remote.proposal_invalid")
    for product in PRODUCTS:
        candidate_cid = str(products[product])
        candidate_name = f"release-inputs/candidates/{product}/{candidate_cid}.json"
        candidate_entry = by_name.get(candidate_name)
        if candidate_entry is None:
            raise RemoteProofError("remote.release_input_missing", candidate_name)
        candidate = strict_json(candidate_entry[1], "remote.candidate_invalid")
        source = sources.get(product)
        vcs = candidate.get("vcs") if isinstance(candidate, dict) else None
        if (not isinstance(candidate, dict) or set(candidate) != CANDIDATE_FIELDS
                or candidate.get("schema") != "suite.product_release_candidate"
                or candidate.get("major") != 1 or candidate.get("cid") != candidate_cid
                or content_cid(candidate) != candidate_cid or candidate.get("product") != product
                or candidate.get("protocol") != candidate_tuple["protocol"]
                or not isinstance(source, dict) or not isinstance(vcs, dict)
                or any(vcs.get(key) != source.get(key) for key in ("commit", "tree", "branch"))):
            raise RemoteProofError("remote.candidate_invalid", product)
        previous = candidate.get("previous_candidate")
        seen = {candidate_cid}
        while previous is not None:
            previous_id = str(previous)
            previous_name = f"release-inputs/candidates/{product}/{previous_id}.json"
            previous_entry = by_name.get(previous_name)
            if previous_id in seen or not CID.fullmatch(previous_id) or previous_entry is None:
                raise RemoteProofError("remote.candidate_lineage_invalid", product)
            seen.add(previous_id)
            previous_candidate = strict_json(previous_entry[1], "remote.candidate_invalid")
            if (not isinstance(previous_candidate, dict) or set(previous_candidate) != CANDIDATE_FIELDS
                    or previous_candidate.get("cid") != previous_id
                    or content_cid(previous_candidate) != previous_id
                    or previous_candidate.get("product") != product):
                raise RemoteProofError("remote.candidate_lineage_invalid", product)
            previous = previous_candidate.get("previous_candidate")
...
def prepare(tuple_cid: str, output: Path) -> dict[str, object]:
    ...
    if (load_release_candidate_tuple is None or validate_product_release_candidate is None
            or load_compatibility_proposal is None or runtime_closure_cas_paths is None
            or load_product_runtime_closure is None or protocol_digest is None):
        raise RemoteProofError("remote.source_commands_unavailable")
    try:
        candidate_tuple = load_release_candidate_tuple(
            tuple_cid, protocol_digest=protocol_digest(), verify_sources=True,
        )
    ...
    verify_driver_interface(ROOT, identities["fusion"]["commit"])
    ...
    for name in SOURCES:
        ...
        verify_checkout(roots[name], identities[name]["commit"], identities[name]["tree"],
                        identities[name]["branch"], require_clean=True)
        bundle = source_bundle(roots[name], identities[name]["commit"], name)
        rows.append(bundle)
    tuple_path = candidate_tuple_path(tuple_cid)
    rows.append((f"release-inputs/candidate-tuples/{tuple_path.name}", 0o644,
                 tuple_path.read_bytes()))
    proposal_path = COMPATIBILITY_PROPOSALS / f"{candidate_tuple['compatibility_proposal']}.json"
    rows.append((f"release-inputs/compatibility-proposals/{proposal_path.name}", 0o644,
                 proposal_path.read_bytes()))
    for product in PRODUCTS:
        candidate_store = PRODUCT_ROOTS[product] / ".deify/releases/candidates"
        candidate_id: str | None = str(products[product])
        seen: set[str] = set()
        while candidate_id is not None:
            if candidate_id in seen or not CID.fullmatch(candidate_id):
                raise RemoteProofError("remote.candidate_lineage_invalid", product)
            seen.add(candidate_id)
            candidate_path = candidate_store / f"{candidate_id}.json"
            candidate_value = strict_json(candidate_path.read_bytes(), "remote.candidate_invalid")
            if (not isinstance(candidate_value, dict) or set(candidate_value) != CANDIDATE_FIELDS
                    or candidate_value.get("cid") != candidate_id
                    or content_cid(candidate_value) != candidate_id or candidate_value.get("product") != product):
                raise RemoteProofError("remote.candidate_lineage_invalid", product)
            rows.append((f"release-inputs/candidates/{product}/{candidate_path.name}", 0o644,
                         candidate_path.read_bytes()))
            previous = candidate_value.get("previous_candidate")
            candidate_id = str(previous) if previous is not None else None
    for product in PRODUCTS:
        platform_closures = closure_maps[product]
        assert isinstance(platform_closures, dict)
        for closure_cid in sorted(set(str(value) for value in platform_closures.values())):
    ...
    if extract:
        ...
        try:
            for name, mode, body in read_tar(data):
                ...
                write_once(destination, body, mode)
            harden_runtime_closure_cas(extract)
            sources = manifest.get("sources")
  ...
  native-intel:
    ...
    steps:
      ...
      - name: Run offline tuple-bound observation
        ...
        run: |
          set +e
          python3 "$RUNNER_TEMP/installed/libexec/fusion/tools/clean_reproduce.py" --release-input-root "$RUNNER_TEMP/export/release-inputs" --tuple "${{ inputs.tuple }}" --platform darwin-x86_64 --runtime-closure-root "$RUNNER_TEMP/export/runtime-closures" --output "$RUNNER_TEMP/observation"
          printf '%s\n' "$?" > "$RUNNER_TEMP/observation-exit-code"
          exit 0
      - name: Upload raw observation
        uses: actions/upload-artifact@65462800fd760344b1a7b4382951275a0abb4808
        with:
          name: "omega-native-intel-raw-${{ inputs.source_sha256 }}"
          path: |
            ${{ runner.temp }}/observation
            ${{ runner.temp }}/observation-exit-code
          if-no-files-found: error
          retention-days: 30
          compression-level: 0
      - name: Require passing observation
        run: test "$(cat "$RUNNER_TEMP/observation-exit-code")" = 0
...
def prepare_repo(transfer: Path, output: Path) -> dict[str, object]:
    ...
    target, transfer_sha = chunked_input(transfer.read_bytes(), output)
...
def prepare_public_repo(transfer: Path, output: Path, key: str | None = None) -> dict[str, object]:
    ...
    if output.exists() or output.is_symlink():
        raise RemoteProofError("remote.repo_output_exists")
    workflow = output / ".github/workflows" / WORKFLOW
    target, transfer_sha = chunked_input(encrypted, output)
...
def dispatch_command(repo: str, ref: str, tuple_cid: str, source_path: str, source_sha: str) -> list[str]:
    ...
    if not CID.fullmatch(tuple_cid):
        raise RemoteProofError("remote.tuple_unpinned")
    if not SHA.fullmatch(source_sha) or source_path != f"input/transfer-{source_sha}.parts.json":
        raise RemoteProofError("remote.artifact_digest_invalid")
...
def verify_row(row: Path, tuple_cid: str, product: str) -> dict[str, object]:
    ...
    return {"product": product, "platform": TARGET,
            "observation": str(observation["cid"]),
            "environment": str(environment["cid"]),
            "detail": digest(payloads["platform-proof.json"]),
            "transcript": digest(payloads["transcript.json"]),
            "lifecycle": digest(payloads["lifecycle.json"])}
...
def parser() -> argparse.ArgumentParser:
    ...
    one = commands.add_parser("assemble-transfer"); one.add_argument("--index", type=Path, required=True); one.add_argument("--source-sha256", required=True); one.add_argument("--output", type=Path, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.command == "assemble-transfer": result = assemble_transfer(args.index, args.source_sha256, args.output)
